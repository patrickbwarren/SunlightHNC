...
\item\verb+dpd_potential(charge_type)+ -- Sets up the DPD potential
  ...
  multiple times.  The optional parameter indicates whether the
  potential should be initialised with Gaussian charges (\verb+1+),
  Bessel charges (\verb+2+), linear (Groot) charges (\verb+3+), Slater
  charges with exact interaction (\verb+4+), or Slater charges
  with approximate interaction (\verb+5+).  The linear case
...
%
\item\verb+urpm_potential(use_ushort_flag)+ -- Sets up the ultrasoft
  ...
  $z_1=1$ and $z_2=-1$.  The optional flag indicates whether the
  potential should be formulated to contain a short range real space
  correction (\verb+use_ushort_flag = 1+).
%
\item\verb+rpm_potential(use_ushort_flag)+ -- Sets up the restricted
  ...
  $z_2=-1$.  The optional flag \verb+use_ushort_flag+ works the same
  way as described for the softened URPM potential.
%
...
\item\verb+msa_solve+ -- Calculate the MSA solution, using a similar
  ...
  \verb+cold_start+ is reset. In the case of a warm start,
  $c'_{\mu\nu}$ outwith the hard core regions is set equal to $-\beta
...
\item\verb+save_reference+ -- In the presence of hard cores, the EXP
  ...
  potential, however this is not enforced.  See next entry for a note on
  the usage.
%
\item\verb+exp_refine+ -- Implement the EXP refinement.  Use
  ...
  three \verb+make_*+ functions are also called.  A call to
  \verb+initialise+ sets $\href_{\mu\nu}=0$, thus \verb+exp_refine+ is
  suitable for immediate use with soft potentials without hard cores.
  To use \verb+exp_refine+ for potentials with hard cores we need to
  calculate and save the reference state, thus (in \FORTRAN\ 90):
%
\begin{verbatim}
...
\end{verbatim}
%
For hard core potentials we need to recalculate $\href_{\mu\nu}$ every
time we call \verb+exp_refine+, since \verb+exp_refine+ sets
$\href_{\mu\nu}=0$ before exiting (for ease of use with soft
potentials).
%
...
\item\verb+c(:, :, :)+ and \verb+e(:, :, :)+ -- Arrays containing the
  ...
  example \verb+c(i, s, p)+ contains $c'_{\mu\nu}(r_i)$, where
  \verb+s+ is the species pair index as in \Eqref{eq:ij}, and \verb+p+
...
\item\verb+h0(:, :)+ -- An array containing the reference state, such
  that \verb+h0(j, s)+ contains $\href_{\mu\nu}(r_i)$ where
  \verb+s+ is the species pair index. 
%
...
\item\verb+ulongk(:, :)+ -- Array containing the Fourier transform of
  ...
  species index.
%
\item\verb+potential_type+ -- The last used potential type. Currently
  set to:
\begin{verbatim}
1 -- 5 : dpd_potential, corresponds to charge_type
10, 11 : urpm_potential without, with use_ushort_flag
20, 21 : rpm_potential without, with use_ushort_flag
\end{verbatim}
%
...
convenient.
%
\item\verb+USE_GAUSSIAN+, \verb+USE_BESSEL+, \verb+USE_LINEAR+,
  \verb+USE_SLATER_APPROX+ and\\ \verb+USE_SLATER_EXACT+ are integer
  constants which can be used for\\ \verb+charge_type+ when
  calling \verb+dpd_potential+.
%
\item\verb+USE_USHORT+ is likewise an integer constant ($=1$) which
  can be used for \verb+use_ushort_flag+ in
  \verb+urpm_potential+ and \verb+rpm_potential+.
  %
\end{itemize}
...
\item\verb+hm_fig4-2_compare.py+ solves for the virial and
  compressibility route pressure for the MSA and the HNC for hard

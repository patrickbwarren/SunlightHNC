...
\item\verb+dpd_potential(charge_type)+ -- Sets up the DPD potential
  ...
  approximate interaction (\verb+charge_type = 5+).  The linear
...
%
\item\verb+soft_rpm_potential(use_ushort)+ -- Sets up the vanilla RPM
  or softened RPM potential described in section \ref{sec:softRPM}.
  This routine can be called multiple times.  It checks that
  \verb+ncomp = 2+ and forces $z_1=1$ and $z_2=-1$.  The control
  parameter \verb+use_ushort+ works the same way as described for the
  softened URPM potential.
%
...
\item\verb+exp_refine+ -- Implement the EXP refinement.  Use
  ...
  calculate and save the reference state, thus (in \FORTRAN):
%
\begin{verbatim}
...
\item\verb+ng+ -- The real space grid size, $n$, usually a power of 2
  (default 4096).
%
\item\verb+nps+ -- The length of the Ng trajectory (cyclical history
  array) (usually safe to leave at the default 6).
%
...
\item\verb+ulongk(:, :)+ -- Array containing the Fourier transform of
  the long range part of the potential, such that \verb+ulongk(j, n)+
...
Note that
the HNC, MSA and RPA solvers require \emph{only} the long range
potential in reciprocal space, $\beta{\tilde v}\lr_{\mu\nu}$
(\verb+ulongk+), and the exponentiated short range potential,
$\exp(-\beta v'_{\mu\nu})$ (\verb+expnegus+), thus a very minimalist
approach can be taken.  The EXP solver uses in addition the long range
potential in real space, $\beta v\lr_{\mu\nu}$ (\verb+ulong+).  The
thermodynamics calculations require all the remaining potential
functions.

...
\end{equation}
%
The equivalent Gaussian smearing length is here $\sigma=\lambda$.  The
...
This actually corresponds to a charge density $\beta^2 e^{-2\beta
  r}/\pi r$.  Originally, Gon\-z\'a\-lez-Melchor \etal\  \cite{GM+06} had
...
closer to the exact result.  The reciprocal space
interaction corresponding to \Eqref{eq:slaterapprox} is
%
\begin{equation}
...
\item\verb+dpd_potential(charge_type)+ -- Sets up the DPD potential
  ...
  linear charges (\verb+charge_type = 3+), or Slater charges
  (\verb+charge_type = 4+ for exact interaction, or \verb+5+ for
  approximate interaction).  The last three cases are incomplete in
  the sense that the thermodynamics is not calculated correctly.
%

...
\item\verb+deficit+ -- The quantity $\aex - \sum_{\mu}\rho_\mu
  \muex_\mu + \pex$ (which should vanish).  This is a certificate of
...
%
\item\verb+vlm_fig2_compare.py+ -- Compute the log activity,
  $\log_{10}\gamma_\infty = (\muex_1 - \muex_0) / \ln 10$, in the infinite
  dilution limit for a binary DPD fluid, and compare to empirical fit
  $0.144 \times \Delta a$ as in Fig.~2 in \Refcite{VLN13}.  The fit
  is derived by these authors from Monte-Carlo trial particle
  insertion simulations (\ala\ Widom).  Note this is essentially the
  same calculation as the preceding example except that $a=106.5$ in
  \Refcite{VLN13}, rather than $a=25$.  For such a large base
  repulsion amplitude, HNC doesn't converge from a cold start, and
  this example illustrates the trick of warming up the solver by
  ramping $a$ up in stages.
%
\item\verb+x_dmu_compare.py+ -- In an unpublished extension to the
  ...
  trial particle identity swap simulations.  The agreement between HNC
  ...
  2x)$ where $\chi$ is the Flory $\chi$-parameter.  This is the basic
...
\item\verb+rpm_oneoff.py+\,, \verb+hs_oneoff.py+ are similar drivers
  for the RPM and hard sphere systems.
%

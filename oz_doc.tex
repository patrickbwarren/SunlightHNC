...
\fbox{\parbox{0.8\textwidth}{The code and this accompanying
    documentation is released under the GPL (v3).  You are welcome to
    use and modify it for your own projects. If you end up publishing
    results based on this, please cite \Refcite{WVA+13}.}}
\end{center}
...
%
\begin{equation}
{\tilde C}' = (I+R\cdot{\tilde H})^{-1}\cdot{\tilde H}
 + \beta{\tilde U}\lr\,.
\label{eq:ozndb}
\end{equation}
%
...
index $n$ by the following rule (the proof is left as an exercise),
%
\begin{equation}
...
\end{equation}
%
This is not used in the code though.
...
by $LU$ decomposition, followed by back-substitution.  The
$LU$ decomposition is done using Crout's algorithm, pivoting on the
diagonal elements.  For more details see \Refcite{NR92}.
...
\begin{itemize}
%
\item\verb+ncomp+ -- The number of component species, ${}\ge 1$ (default 1).
%
...
\item\verb+gw_p_compare.py+ -- calculate the virial route pressure as
  ...
  which can be compared directly with Fig.~4 of \Refcite{GW97}.  The
...
\item\verb+wsg_fig5_compare.py+ -- calculate the free energy
  ...
  introduced in \Refcite{WSG01} and is representative of an
  ...
  \Refcite{WSG01}.  The agreement is to within 4\%, up to
  $\Delta a=25$.
%

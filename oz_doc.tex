...
\noindent{\bf\large FORTRAN 90 Ornstein-Zernike solver --- version 1.11}\\[6pt]
...
\vspace{0.25in}
\noindent 
Last two point releases fix bugs in the treatment of hard cores: the contact
virial pressure contribution was missing from the HNC free energy
(fixed in v1.10), and the compressibility and HNC chemical potentials
was missing a mean-field term for the off-SYM RPM potential (fixed in
v1.11).\\

\noindent Features of version 1.9:
\begin{itemize}
...
\noindent Added in version 1.9:
\begin{itemize}
\item EXP  for hard core potentials fixed, new examples added ;
\item Source code brought up to a modern \FORTRAN\ 90 standard.
\end{itemize}

\noindent Added in versions 1.8 and 1.7:
\begin{itemize}
\item MSA closure and EXP ;
...
\Eqref{eq:slaterapprox}, but the correct mapping
$\varbeta=5/(8\lambda)$ makes \Eqsref{eq:slaterapprox} and
\eqref{eq:slaterexact} have the same limiting value as $r\to0$.
...
%
\item\verb+model_name+ -- String (character array) describing the
  potential model last used to set the potential arrays.
%
\item\verb+version+ -- String (character array) giving the
  SunlightHNC version number (added in 1.10).
%
\end{itemize}

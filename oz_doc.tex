...
\noindent{\bf\large FORTRAN 90 Ornstein-Zernike solver :: version 1.12}\\[6pt]
\noindent{\it Patrick B Warren, Unilever R\&D Port Sunlight (February
  2018)}\\[6pt]
...
%
\begin{equation}
\frac{\Uex}{V}=2\pi\rho\,{\textstyle \sum_{\mu\nu}}\,\rho x_\mu x_\nu 
...
\end{equation}
%
The first (real space) term in this is simplified from the literature
expressions (\cf\ Eq.~(5.1) in \Refcite{Hir60} or Eq.~(30) in
\Refcite{LB94}), by assuming the HNC closure \Eqref{eq:hncnd}.  The
...
the matrix with entries ${\tilde c}_{\mu\lambda}$.  For the real space
part we make the usual split so that $\beta\aex{}^{,1} = (\rho/2)\,
{\textstyle \sum_{\mu\nu}}\,\rho x_\mu x_\nu t^{\mathrm{A}}_{\mu\nu}$
where
%
\begin{equation}
...
adsorbed into a redefinition of the short range potential.  The \emph{default}
...
%
\begin{equation}
\beta {\tilde v}\lr(k)=\frac{4\pi\lB}{k^2}\times\frac{1}{(1+k^2/4\varbeta^2)^2}\,.
\end{equation}
%
...
Note that by writing $\beta {\tilde v}\lr(k)=4\pi\lB
k^{-2}(1+k^2\sigma^2/n)^{-n}$, there is a natural hierarchy going from
Bessel ($n=1$), to approximate Slater ($n=2$), to exact Slater
($n=4$), and to Gaussian ($n\to\infty$).
...
therein.  The eigenvalues of the matrix required for the HNC
free energy are found by calling \verb+DGEEV+  in the \LAPACK\ library
(general purpose, since the matrix is not symmetric).
...
\item\verb+cf_mf+\,, \verb+cf_gc+\,, \verb+cf_xc+ -- The mean-field,
  ...
  route compressibility factor $\beta p/\rho$.
%
...
\item\verb+ulongk(:, :)+ -- Array containing the Fourier transform of
  ...
  species index.
%
\item \verb+pi+\,, \verb+twopi+\,, \verb+fourpi+ --- Available for convenience
($=\pi, 2\pi, 4\pi$).
%
\end{itemize}
...
single-component DPD, ${\partial\Aex}/{\partial A}={\langle
  V\rangle}/{A}$ where $\langle V\rangle\equiv U$ is the internal
energy.  Thus
%
\begin{equation}
\Aex=\int_0^A\frac{dA'}{A'}\,\langle V\rangle_{A'}
\label{eq:fint}
\end{equation}
%
...
is a good test of convergence (the absolute HNC deficit is
$2.6\times10^{-4}$).  Sections \ref{sec:vrp}--\ref{sec:feng} have been
coded up as \verb+examples.py+.
...
%
\item\verb+urpm_wertheim_*.*+ are scripts to solve the URPM
  coexistence problem using HNC for the high density phase boundary
  and Wertheim theory with a softened URPM reference fluid (solved by
  HNC) for the low density phase boundary.  For more details and
  sample results see comments in the scripts.
%
